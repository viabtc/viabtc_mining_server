...
# define QUEUE_SHM_KEY      0x16120701
...
static int init_clt(void)
{
    ...
    clt = rpc_clt_create(&settings.writer, &type);
    if (clt == NULL)
        return -__LINE__;
...
{
    int ret = queue_init(&queue, "writer", QUEUE_SHM_KEY, 10 * 1024 * 1024, settings.queue, 1024 * 1024 * 1024);
    if (ret < 0) {
