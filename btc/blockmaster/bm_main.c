...
int main(int argc, char *argv[])
{
    ...
    if (process_exist("blockmaster.exe") != 0) {
        printf("process exist\n");
        exit(EXIT_FAILURE);
    }

    process_title_init(argc, argv);
    process_title_set("blockmaster.exe");
    ...
    ret = init_master();
    if (ret < 0) {
        error(EXIT_FAILURE, errno, "init master fail: %d", ret);
    }
    ret = init_cli();
    if (ret < 0) {
    ...
    ret = init_tx();
    if (ret < 0) {
        error(EXIT_FAILURE, errno, "init tx fail: %d", ret);
    }
    ret = init_block();
    if (ret < 0) {
