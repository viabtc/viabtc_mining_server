...
int load_cfg_redis(json_t *root, const char *key, redis_cfg *cfg)
{
    ...
    ERR_RET(read_cfg_int(node, "port", &cfg->port, false, 6379));
    ERR_RET(read_cfg_str(node, "auth", &cfg->auth, ""));
...
static int load_cfg_coin_rpc_daemon(json_t *root, coin_daemon *daemon)
{
    ERR_RET(read_cfg_str(root, "host", &daemon->host, NULL));
    ERR_RET(read_cfg_int(root, "port", &daemon->port, false, 0));
    ERR_RET(read_cfg_str(root, "user", &daemon->user, NULL));
    ERR_RET(read_cfg_str(root, "pass", &daemon->pass, NULL));

    return 0;
}

int load_cfg_coin_rpc_sub(json_t *node, coin_rpc_cfg *cfg)
{
    ERR_RET(read_cfg_str(node, "name", &cfg->name, NULL));
    json_t *addr = json_object_get(node, "addr");
    if (!addr) {
        cfg->count = 1;
        cfg->arr = malloc(sizeof(coin_daemon));
        ERR_RET(load_cfg_coin_rpc_daemon(node, &cfg->arr[0]));
    } else if (json_is_object(addr)) {
        cfg->count = 1;
        cfg->arr = malloc(sizeof(coin_daemon));
        ERR_RET(load_cfg_coin_rpc_daemon(addr, &cfg->arr[0]));
    } else if (json_is_array(addr)) {
        cfg->count = json_array_size(addr);
        if (cfg->count == 0)
            return -__LINE__;
        cfg->arr = malloc(sizeof(coin_daemon) * cfg->count);
        for (uint32_t i = 0; i < cfg->count; ++i) {
            json_t *row = json_array_get(addr, i);
            if (!row || !json_is_object(row))
                return -__LINE__;
            ERR_RET(load_cfg_coin_rpc_daemon(row, &cfg->arr[i]));
        }
    } else {
        return -__LINE__;
    }
...
int load_cfg_coin_rpc(json_t *root, const char *key, coin_rpc_cfg *cfg)
{
    json_t *node = json_object_get(root, key);
    if (!node || !json_is_object(node))
        return -__LINE__;

    return load_cfg_coin_rpc_sub(node, cfg);
}
...
int load_cfg_inetv4_list_direct(json_t *node, inetv4_list *cfg)
{
    if (!node || !json_is_array(node))
        return -__LINE__;
